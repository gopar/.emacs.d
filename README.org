...
(use-package python-isort
  :ensure t
  :hook (python-mode . gopar/enable-isort-if-found)
  :init
  (defun gopar/enable-isort-if-found ()
    "Format the current buffer using the `isort' program, if available."
    (interactive)
    (if (executable-find "isort")
        (python-isort-on-save-mode)
      (message "isort not found. Not enabling `isort-mode`"))))
#+end_src
...
(use-package web-mode
  :ensure t
  :bind (:map web-mode-map ("M-." . gopar/html-jump-to-template-def))
  :init
  (setq-default web-mode-code-indent-offset 2)
  (setq web-mode-engines-alist '(("django"    . "\\.html\\'")))

  (defun gopar/html-jump-to-template-def ()
    "When in web-mode, we can use this function to jump to template definition"
    (interactive)
    (require 'cl-lib)
    (let (path string-at-point)
      (setq string-at-point (thing-at-point 'filename t))
      (setq path (cl-find-if (lambda (x) (string-match-p string-at-point x)) (projectile-project-files (projectile-project-root))))

      (if path
        (progn
          (xref--push-markers)
          (find-file (format "%s%s" (projectile-project-root) path)))
...
(use-package devdocs
  ...
  :hook ((python-mode . (lambda () (setq-local devdocs-current-docs '("django~4.2" "django_rest_framework" "python~3.10"))))
         (web-mode . (lambda () (setq-local devdocs-current-docs        '("vue~3" "vue_router~4" "javascript" "typescript" "vitest" "moment" "tailwindcss" "html"))))
         (typescript-mode . (lambda () (setq-local devdocs-current-docs '("vue~3" "vue_router~4" "javascript" "typescript" "vitest" "moment"))))))
#+end_src
