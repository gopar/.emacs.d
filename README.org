...
(use-package hydra
 ...
 Move: _s_wap _a_ce _B_alance windows
...
(use-package gpt-commit
  ...
  :custom
  (gpt-commit-openai-key (auth-source-pick-first-password :host "api.openai.com")))
#+end_src
...
(defun open-chatgpt-shell-window ()
  "Open a new window for chatgpt-shell."
  (interactive)
  (let ()
    ;; Start chatgpt
    (chatgpt-shell)
