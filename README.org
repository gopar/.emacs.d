...
(use-package consult
  ...
  :init
  (setq consult-themes (remove 'light-blue (remove 'tao (custom-available-themes))))
...
(use-package dashboard
  ...
  (defun gopar/dashboard-insert-vocabulary (list-size)
    (dashboard-insert-heading "Vocabulary of the Day:"
                              nil
    ...
    (let ((random-line nil)
      ...
      (insert "    " random-line)))
  :config
  (add-to-list 'dashboard-item-generators  '(vocabulary . gopar/dashboard-insert-vocabulary))
  (add-to-list 'dashboard-items '(vocabulary) t)
...
(use-package chatgpt-shell
  ...
  (defun gopar/chatgpt-shell ()
    "Open a new dedicated window for chatgpt-shell."
    (interactive)
    (if (chatgpt-shell--primary-buffer)
        (kill-buffer (chatgpt-shell--primary-buffer))

      ;; Create a new window on the right side with size of 60
      (split-window-horizontally (- (window-width) 60))
      (other-window 1)
      (switch-to-buffer (chatgpt-shell--primary-buffer))
      (chatgpt-shell)
      ;; Dedicate the window so it cannot be closed
...
(use-package tao-theme
  :ensure t
  ;; :hook (after-init .  (lambda () (load-theme 'tao-yin t)))
  :custom
...
(use-package haki-theme
  :ensure t
  :hook (after-init .  (lambda () (load-theme 'haki t))))
#+end_src
*** Doom Themes
           ...
           (cond
            ...
            ((and (condition-case _err
                  ...
                  (or (string-prefix-p "*eshell" (buffer-name))
                      (string-prefix-p "*vterm" (buffer-name)))) (list (projectile-project-name)))
...
Use `golden-ratio-toggle-widescreen` if splits are too wides
#+begin_src emacs-lisp
