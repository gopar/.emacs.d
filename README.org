...
(use-package org
  ...
  (org-refile-targets '(("~/.emacs.d/org/private/gtd/gtd.org" :maxlevel . 3)
                        ("~/.emacs.d/org/private/gtd/someday.org" :level . 1)
                        ("~/.emacs.d/org/private/gtd/tickler.org" :maxlevel . 1)))
...
(use-package org-agenda
  ...
  ;; Start the view on sunday so I can get an overview of upcoming week
...
(use-package org-capture
  ...
  (org-capture-templates
   '(
     ("c" "Inbox" entry (file "~/.emacs.d/org/private/gtd/inbox.org")
      "* TODO %? \n %U")
     ("e" "Inbox [Emacs]" entry (file "~/.emacs.d/org/private/gtd/inbox.org")
      "* TODO %? :freetime:emacs:\n %U")
     ("p" "Project" entry (file "~/.emacs.d/org/private/gtd/gtd.org")
      "* PROJ %? [%] :project: \n:PROPERTIES: \n:TRIGGER: next-sibling todo!(NEXT) \n:ORDERED: t \n:END: \n%U \n** TODO Add entry")
     ("w" "Work" entry (file "~/.emacs.d/org/work/work.org")
      "* TODO %? :work:\n %U")
     ("t" "Tickler" entry (file "~/.emacs.d/org/private/gtd/tickler.org")
...
(use-package org-roam
  ...
  (org-roam-capture-templates
   '(("d" "default" plain #'org-roam-capture--get-point
      ...
      :head "#+title: ${title}\n"
      :unnarrowed t)))
  (org-roam-dailies-directory (expand-file-name "./org/private/journal/"))
  (org-roam-dailies-capture-templates
   `(("d" "daily" plain
      #'org-roam-capture--get-point
      (file ,(concat user-emacs-directory "org/templates/dailies-daily.template"))
      :file-name ,(concat org-roam-dailies-directory "daily/%<%Y-%m-%d>")
      :head "#+title: %<%Y-%m-%d>\n"
      :unnarrowed t)
     ("w" "weekly" plain
      #'org-roam-capture--get-point
      (file ,(concat user-emacs-directory "org/templates/dailies-weekly.template"))
      :file-name ,(concat org-roam-dailies-directory "weekly/%<%Y-%m-%d>")
      :head "#+title: %<%Y-%m-%d>\n")
     ("m" "monthly" entry
      #'org-roam-capture--get-point
      "* %?"
      :file-name ,(concat org-roam-dailies-directory "monthly/%<%Y-%m>")
      :head "#+title: %<%Y-%m>\n")))
...
(use-package erc
  ...
  (defun my/erc-start-or-switch ()
    ...
    (if (get-buffer "irc.freenode.net:6667")
      ...
      (erc :server "irc.freenode.net" :port 6667 :nick "gopar")))
  ...
  :custom
  (erc-autojoin-channels-alist '(("freenode.net" ;; "#emacs" "#emacs-beginners" "#emacsconf-org"
                                  "#python" ;; "#docker""#stumpwm" "#django" "#fsf" "#org-mode"
                                  )))
...
(defun gopar/add-env-vars ()
  ...
  (setenv "PATH" (concat (getenv "PATH") (concat ":" (expand-file-name "~/.nvm/versions/node/v12.20.0/bin"))))
  ...
  (setenv "NVM_PATH" (expand-file-name "~/.nvm/versions/node/v12.20.0/bin/node"))
  (setenv "NVM_BIN" (expand-file-name "~/.nvm/versions/node/v12.20.0/bin"))
  ...
  (setq exec-path (append exec-path
                          '("/usr/local/bin"
                            ...
                            "/sbin"
                            "/bin"
                            (expand-file-name "~/.nvm/versions/node/v12.20.0/bin")))))
...
(use-package request
  ...
  (request-log-level -1)
  :config
  (advice-add 'eshell :after '(lambda ()
                                (funcall (nth (random (length gopar/request-jokes)) gopar/request-jokes))))
  :init
  (defun gopar/request-chuck-norris-joke ()
    "Sets a chuck norris joke on next eshell launch"
    (interactive)
    (request
     ...
     :success (cl-function (lambda (&key data &allow-other-keys)
                             (setq eshell-banner-message (concat (assoc-default 'value data) "\n\n"))))))

  (defun gopar/request-dad-joke ()
    "Sets a dad joke on next eshell launch"
    (interactive)
    (request
     ...
     :headers '(("Accept" . "application/json"))
     :success (cl-function (lambda (&key data &allow-other-keys)
                             (setq eshell-banner-message (concat (assoc-default 'joke data) "\n\n"))))))

  (setq gopar/request-jokes '(gopar/request-chuck-norris-joke gopar/request-dad-joke))
  ;; Lets set a joke since we're already doing all these stuffs
  (request
   "https://api.chucknorris.io/jokes/random"
   :parser 'json-read
   :headers '(("Accept" . "application/json"))
