...
(use-package consult
  :ensure
  :after projectile
  :hook (after-init . (lambda () (load-theme 'doom-shades-of-purple) ;; (load-theme (nth (random (length consult-themes)) consult-themes))
                        ))
  ...
  ;; Any themes that are incomplete/lacking
...
#+begin_src emacs-lisp
(use-package haki-theme :ensure t)
#+end_src
